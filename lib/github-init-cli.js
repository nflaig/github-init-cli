...
const setCredentials = (username, password) => {
    ...
    logger.info("Username set to " + info(username));
    logger.info("Password set to " + info(password));
};
...
const setWorkingDir = (path) => {
    ...
    logger.info("Working directory set to " + info(path));
};

const create = async (repositoryName) => {
    ...
    try {
        ...
        logger.info("Please set your GitHub credentials by running " + info("github setCredentials <username> <password>"));
        process.exit(1);
    }
